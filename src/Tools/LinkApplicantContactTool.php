...
class LinkApplicantContactTool implements ToolContract, ToolMetadataContract
{
    ...
    {
        return 'hcm.applicant_contacts.POST';
    }

    public function getDescription(): string
    {
        return 'POST /hcm/applicants/{applicant_id}/contacts - Verknuepft einen bestehenden CRM Contact mit einem Bewerber. Parameter: applicant_id (required), contact_id (required).';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        try {
            ...
            $found = $this->validateAndFindModel($arguments, $context, 'applicant_id', HcmApplicant::class, 'NOT_FOUND', 'Bewerber nicht gefunden.');
            if ($found['error']) {
                return $found['error'];
            }
    ...
    public function getMetadata(): array
    {
        return [
            'read_only' => false,
            'category' => 'action',
            'tags' => ['hcm', 'applicant', 'crm', 'link', 'create'],
            'risk_level' => 'write',
            'requires_auth' => true,
            'requires_team' => true,
            'idempotent' => true,
